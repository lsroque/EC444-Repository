...
// I2C Display of ASCII Characters
//source: https://github.com/adafruit/Adafruit_LED_Backpack/blob/master/Adafruit_LEDBackpack.cpp
...
static void test_alpha_display() {
    ...
    //Added code to allow user to communicate with the console
    char str[8];
    if (ret == ESP_OK) {printf("Enter a 4 ASCII Characters: ");}
    scanf("%s", str); //store user input
    fflush(stdin);

    printf(" - wrote: %s  \n\n", str);
    printf(" Program over. Press Ctrl + ] to exit. \n\n");
    uint16_t displaybuffer[8]; //or 128??
    for (int i = 0; i < 9; i++) {
      if(str[i+1] == '.') { //ASCII character is '.'
        //Attempt to display decimal, code works with ASCII characters and digits
      }
      else {
        displaybuffer[i] = alphafonttable[(uint16_t)str[i]];
      }
      // for loop finds each letter character in the table, sets displaybuffer,
      // and displays
    }
